<?php
    ...
    if (isset($_POST['name']) && $_POST['name'] != "")
    {
        ...
        $resultn = $mysqli->query("SELECT * FROM user WHERE name='$name'");
        if ($resultn->num_rows == 0)
        {
          $name_validate=' is-valid';
          $name_value=' value="'.$name.'"';
          
          $err_name='<div class="valid-feedback">
          Ime --'.$name.'-- je slobodno.
          </div>';
        }
        elseif ($resultn->num_rows > 0)
        {
          ...
          while ($resultn->num_rows != 0)
          {
            echo $name2=$name.$f;
            $f=$f+1;
            $resultn = $mysqli->query("SELECT * FROM user WHERE name='$name2'");
          }
          $name_value='value="'.$name2.'"';
          $err_name='<div class="invalid-feedback">'.$name.'
          je zauzeto, probajte sa: <strong class=" is-valid">'.$name2.'</strong></div>';
